...
#endif
#include "com_type.h"
#include "monitor.h" /* FIXME memory pool should not band to specific module */
#include "mempool.h"
...
/*
 * @brief    create a new node by offset
 * @param    p_mem_base -i- address of memory base
 * @return   pointer to new node if success, else return a NULL
 * @note     this api is used for those cannot use pointer to specify the
 *           element position, e.g. shared memory
 */
...
void mp_update_pool(mp_i32 *p_mem_base)
{
	...
	p_mem_head->p_node_first = (struct mp_node_t *)(p_mem_head + 1) +
		p_mem_head->node_first_offset;
	p_mem_head->p_node_tail = (struct mp_node_t *)(p_mem_head + 1) +
		p_mem_head->node_tail_offset;
